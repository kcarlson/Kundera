 ...
 * @see <a
 *      href="http://raw.github.com/edanuff/CassandraCompositeType">Taken from 
 *      Ed Anuff's github</a>
 */
...
public class Composite implements Collection<Object>, Comparable<Composite>
{
    ...
    public String toString()
    {
        ...
        while (iter.hasNext())
        {
            ...
            {
                builder.append(";").append(COMPONENT_REAL);
            }
            ...
            {
                builder.append(";").append(COMPONENT_BOOL);
            }
            else if (o instanceof String)
            {
                builder.append(";").append(COMPONENT_UTF8);
            }
            else if (o instanceof UUID)
            {
                if (isTimeBased((UUID) o))
                {
                    builder.append(";").append(COMPONENT_TIMEUUID);
                }
                else
                {
                    builder.append(";").append(COMPONENT_LEXICALUUID);
                }
            }
    ...
    {
        return (obj instanceof UUID) || (obj instanceof String) || (obj instanceof Long) || (obj instanceof Integer)
    ...
    public static int compare(ByteBuffer b1, ByteBuffer b2)
    {
        ...
        return compare(b1.arrayOffset() + b1.position(), b1.array(), b2.arrayOffset() + b2.position(), b2.array());
    }
    ...
    public static int compare(int s1, byte[] o1, int s2, byte[] o2)
    {
        ...
        while (true)
        {
            ...
            if (type1 != type2)
            {
                ...
                else if ((type2 == COMPONENT_STOP) && (type1 == COMPONENT_MINIMUM))
                {
                    return -1;
                }
            ...
            case COMPONENT_ASCII:
                ...
                comp = compareByteArrays(o1, offset1 + 3, len1, o2, offset2 + 3, len2);
                offset1 += len1 + 3;
                offset2 += len2 + 3;
                break;
    ...
    private static int compareBool(byte[] bytes1, int offset1, byte[] bytes2, int offset2)
    {
    ...
    private static int compareLong(byte[] bytes1, int offset1, byte[] bytes2, int offset2)
    {
    ...
    private static int compareReal(byte[] bytes1, int offset1, byte[] bytes2, int offset2)
    {
    ...
    private static int compareUTF8(byte[] bytes1, int offset1, int len1, byte[] bytes2, int offset2, int len2)
    {
    ...
    private static int compareLexicalUUID(byte[] bytes1, int offset1, byte[] bytes2, int offset2)
    {
    ...
    private static int compareTimestampBytes(byte[] o1, int i1, byte[] o2, int i2)
    {
    ...
    private static int compareTimeUUID(byte[] bytes1, int offset1, byte[] bytes2, int offset2)
    {
    ...
    private static Object deserializeBytesAt(byte[] bytes, int[] offsetRef)
    {
        ...
        default:
            throw new MarshalException("Unknown embedded type at offset: " + offset);
        }
    ...
    public static List<Object> deserialize(ByteBuffer bytes)
    {
        ...
        return deserialize(bytes.arrayOffset() + bytes.position(), bytes.array());
    }
    ...
    public static List<Object> deserialize(int s, byte[] bytes)
    {
        ...
        while (true)
        {
            ...
            if (bytes.length < offset)
            {
                throw new MarshalException("Incorrect number of bytes, either value is corrupt or not composite type");
            }
    ...
    public static boolean validate(ByteBuffer buffer)
    {
        ...
        return validate(buffer.arrayOffset() + buffer.position(), buffer.array(), false);
    }
    ...
    public static boolean validate(ByteBuffer buffer, boolean throwException)
    {
        ...
        return validate(buffer.arrayOffset() + buffer.position(), buffer.array(), throwException);
    }

    public static boolean validate(int start, byte[] bytes, boolean throwException)
    {
        if ((bytes != null) && (bytes.length > (start + 4)))
        {
            ...
            if ((bytes[start] & 0xFF) != COMPOSITETYPE_ID_0)
            {
                if (throwException)
                {
                    throw new MarshalException("Not a composite type (ID byte 0 incorrect)");
                }
                return false;
            }
            ...
            if ((bytes[start + 1] & 0xFF) != COMPOSITETYPE_ID_1)
            {
                if (throwException)
                {
                    throw new MarshalException("Not a composite type (ID byte 1 incorrect)");
                }
                return false;
            }
            ...
            if ((bytes[start + 2] & 0xFF) != COMPOSITETYPE_ID_2)
            {
                if (throwException)
                {
                    throw new MarshalException("Not a composite type (ID byte 2 incorrect)");
                }
                return false;
            }
            ...
            if ((bytes[start + 3] & 0xFF) != COMPOSITETYPE_VERSION)
            {
                if (throwException)
                {
                    throw new MarshalException("Incorrect composite type version for this deserializer, expected "
                            + COMPOSITETYPE_VERSION + ", found " + (bytes[start + 3] & 0xFF));
                }
