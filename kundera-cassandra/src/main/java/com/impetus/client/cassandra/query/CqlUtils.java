 ...
 * 
 * @author cassandra-jdbs
 * @see org.apache.cassandra.cql.jdbc
 */
class CqlUtils
{
    private static final Pattern KEYSPACE_PATTERN = Pattern.compile("USE (\\w+);?", Pattern.CASE_INSENSITIVE
            | Pattern.MULTILINE);
    ...
    protected static final Logger logger = LoggerFactory.getLogger(CqlUtils.class);
    ...
    public static final String createSubName(Properties props) throws SQLException
    {
        ...
        try
        {
            uri = new URI(null, null, host, props.getProperty(TAG_PORT_NUMBER) == null ? DEFAULT_PORT : Integer
                    .parseInt(props.getProperty(TAG_PORT_NUMBER)), keyspace, null, null);
        }
        catch (Exception e)
        {
